...
export function participate<C extends CoValue, R extends RefsToResolve<C>>(
  Schema: CoValueClass<C>,
  id: ID<CoValue>,
  options: { resolve: RefsToResolveStrict<C, R>; prompt: string },
) {
  return subscribeToCoValue(
    Schema,
    id,
    { ...options, loadAs: Account.getMe() },
    async (update) => {
        ...
        [
          {
            role: "system",
            content:
              "You are a helpful assistant who reacts to app state based on the following user prompt: " +
              options.prompt,
          },
          {
            role: "user",
            content: "Current app state: " + JSON.stringify(update.toJSON()),
          },
        ],
        {
          max_new_tokens: 50,
