...
Jazz enables real-time collaboration through its synchronization system. Once you've established your data structure with [schemas](/docs/building-with-jazz/schemas) and set up access control through [groups](/docs/building-with-jazz/groups), understanding how data moves between devices is key to building effective collaborative applications.
...
1. **Local-first operations**: Changes happen immediately on the local device
2. **Background synchronization**: Changes are sent to a sync server
3. **Conflict resolution**: The CRDT nature of CoValues automatically resolves conflicts
...
The simplest option is to use Jazz Cloud, a managed service that provides sync servers for you:
...
For those with specific requirements, Jazz allows you to run your own sync server:
...
Now that you understand how sync works in Jazz, you're ready to learn about [Server Workers](/docs/building-with-jazz/server-workers) - how to add server-side logic to your application.

## Related

For more detailed information about sync, see:
- [Sync Configuration](/docs/sync-and-storage) - Technical details for sync options
