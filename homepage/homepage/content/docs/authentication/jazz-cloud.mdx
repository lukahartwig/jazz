...
<ContentByFramework framework={["react-native-expo"]}>
  <CodeGroup>
    ```sh
    pnpm add jazz-react-auth-betterauth @better-auth/expo expo-secure-store
    ```
  </CodeGroup>
</ContentByFramework>
...
<CodeGroup>
  ...
  declare const providerName: Parameters<typeof auth.authClient.signIn.social>[0]["provider"];
  // ---cut---
