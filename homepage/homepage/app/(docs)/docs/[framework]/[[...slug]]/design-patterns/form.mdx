...
export function CreateOrder() {
  ...
  const onSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const validation = draft.validate(); // [!code focus:5]
...
export class AccountRoot extends CoMap { // [!code focus:15]
...
import { JazzAccount } from "./schema"; // [!code focus]

export function MyJazzProvider({ children }: { children: React.ReactNode }) {
    return (
        <JazzProvider
            sync={{ peer: "wss://cloud.jazz.tools/?key=you@example.com" }}
            AccountSchema={JazzAccount} // [!code focus]
        >
            {children}
        </JazzProvider>
    );
...
declare module "jazz-react" { // [!code focus:5]
    interface Register {
        Account: JazzAccount;
    }
}
...
// CreateOrder.tsx
export function CreateOrder() {
  const { me } = useAccount({ root: { draft: {} } }); // [!code focus:3]
  ...
  const onSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const draft = me.root.draft; // [!code focus:2]
    if (!draft) return;
    ...
    me.root.draft = DraftBubbleTeaOrder.create( // [!code focus:3]
      {},
    );
  };
...
function CreateOrderForm({ // [!code focus:13]
  id,
  onSave,
}: {
