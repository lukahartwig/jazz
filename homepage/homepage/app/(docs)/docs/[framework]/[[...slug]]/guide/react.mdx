...
<CodeGroup>
    ...
    import "./index.css"; 
    import { JazzProvider } from "jazz-react";  // [!code focus]

    ReactDOM.createRoot(document.getElementById("root")!).render( 
        <React.StrictMode> 
            <JazzProvider // [!code focus:6]
...
function App() {
    ...
    const createIssue = () => { // [!code focus:11]
...
import { Issue } from "./schema"; 
import { IssueComponent } from "./components/Issue.tsx"; 
import { useCoState } from "jazz-react"; // [!code focus]
import { ID } from "jazz-tools" // [!code focus]

function App() { 
    const [issue, setIssue] = useState<Issue>(); // [!code --]
    const [issueID, setIssueID] = useState<ID<Issue>>(); // [!code focus]
    
    const issue = useCoState(Issue, issueID); // [!code focus]
...
function App() { 
    const [issueID, setIssueID] = useState<ID<Issue> | undefined>( // [!code focus:3]
    ...
    const createIssue = () => {
        ...
        setIssueID(newIssue.id); 
        window.history.pushState({}, "", `?issue=${newIssue.id}`); // [!code focus]
    }; 
...
import { Issue } from "./schema"; 
import { IssueComponent } from "./components/Issue.tsx"; 
import { useAccount, useCoState } from "jazz-react"; // [!code focus:2]
import { ID, Group } from "jazz-tools"

function App() { 
    const { me } = useAccount(); // [!code focus]
    ...
    const createIssue = () => { 
        const group = Group.create(); // [!code focus:2]
...
<CodeGroup>
```ts
import { CoMap, CoList, co } from "jazz-tools"; // [!code focus]
...
export class ListOfIssues extends CoList.Of(co.ref(Issue)) {} // [!code focus:6]
...
import { IssueComponent } from "./Issue.tsx"; 
import { useCoState } from "jazz-react"; 
import { createInviteLink } from "jazz-react"; // [!code focus]
...
export function ProjectComponent({ projectID }: { projectID: ID<Project> }) {
    ...
    const invite = (role: "reader" | "writer") => { // [!code focus]
        const link = createInviteLink(project, role, { valueHint: "project" }); // [!code focus]
        navigator.clipboard.writeText(link); // [!code focus]
    }; // [!code focus]
    ...
    return project ? (
        <div>
            <h1>{project.name}</h1>
            {me.canAdmin(project) && ( // [!code focus:6]
                <>
